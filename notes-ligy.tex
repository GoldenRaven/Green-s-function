...
I_{s}&= i \sum_{q} J_{q}\left(\left\langle s_{q}^{+} a_{q}^{\dagger}\right\rangle-\left\langle a_{q} s_{q}^{-}\right\rangle\right) \\
&=\sum_{q}J_{q}(G_{d,R}^{<}(t,t)-G_{R,d}^{<}(t,t)) \\
&= -2\rm{Re}\sum_{q} \int  \frac{d\omega}{2\pi}\operatorname{Tr}\left[\left(G_{d}^{>}-G_{d}^{<}\right) \Sigma_{R q}^{<}+G_{d}^{<}\left(\Sigma_{R q}^{a}-\Sigma_{R q}^{r}\right)\right]
\end{split}
\end{equation}
...
\end{equation}
and $\omega = \varepsilon_{\downarrow} - \varepsilon_{\uparrow}, \Delta\mu_{s}=\mu_{\uparrow}-\mu_{\downarrow}$.
